...
$$S=\{3,6,7,9\}\implies B(S)=001001101$$

In other words: it is a "checklist" in the form of $0$s and $1$s.

We are interested in bitvectors because any **integer set** $S$ can be represented as a bitvector $B$ where $B[x]=1$ iff $x\in S$.
Under this representation, we have the following mapping of functions:

| Bitevector operation | Mapped to sets | Mapped to strings |
| :------------------: | :------------: | :---------------: |
|       `Access`       |  `Membership`  |      `Count`      |
|        `Rank`        | `Predecessor`  |     `Locate`      |
|       `Select`       |  `Successor`   |     `Extract`     |

By sets we mean ordered sets (or sets over a numerical universe).
For example considering the example at the beginning, in order to find the successor of $4$, we look for the first $1$ after the fourth index, that is $6$.

### Zero-order compressed bitvectors
There exists a data structure over **any bitvector** $B\in\mathscr B_{n,m}$ using the following space complexity (in bits):
$$\log\binom{n}{m}+o(n)\leq nH_0+o(n)$$ and supporting **constant-time** access, rank, select **queries**.
>Note that we use $o(n)$ instead of $o(\\n\log n)$ that we've seen in the [[02 - Basics of information theory#Higher-order empirical entropy|goal of an entropy compressor]] because now the alphabet is only $0$s and $1$, hence $o(n\log_22)=o(n)$.
...
Using an array of integers ($32n$ bits) or an array of booleans ($8n$ bits) is wasteful, the right approach is **bit-packing**: pack all $n$ bits tightly into $n/w$ integers of $w$ bits each.
...
>
>$w=\log n$ because it is essentially a pointer, and since the _Word RAM model_ assumes that as our data grows, our computer also grows in order to handle that data, we cannot use a fixed-length pointer (like in the reality), hence it must be long $1$ word, but that word has to address the whole length $n$, hence the pointer is $\log n$ bits long.
...
>This is because if $\ell\leq w$ the entire subsequence fits inside one word (considering the word RAM model).

---
### Compressing the bitvector
Our next goal is to compress $B\in\{0,1\}^n$ in $nH_0+o(n)$ bits.

Let $B$ be our bitvector, we choose a **block size** $b=\left\lceil\frac{\log_2(n)}{2}\right\rceil=\Theta(\log n)$ and divide $B$ in blocks of $b$ bits each.
...
With $n=21$ we get $b=\left\lceil\frac{\log_2(21)}{2}\right\rceil=3$ hence we have $21/3=7$ blocks.
$$\begin{array}{ccccccc}
...
1&2&1&2&0&2&3
\end{array}$$
We need $\lceil\log_2(b+1)\rceil$ **bits to represent each integer of the class array** $C$ (we have to represent numbers that go from $0$ to $b$), then by substituting $b$, we get $O(\log\log n)$.
...
Continuing the previous example, we build the lookup table $T$ (containing every possible combination per class):
...
- It has $b+1$ rows, one for each possible number of $1$s
- $\leq2^b$ columns, the total number of unique bit patterns of length $b$
- Each cell has $b$ bits

Is equal to:
$$\text{Rows}\times\text{Columns}\times\text{Cell size }=O(b^2\cdot 2^b)\stackrel{\text{substitute b}}{\Longrightarrow}O((\log n)^2\cdot\sqrt n)=o(n)\text{ bits}$$
...
How many bits do we need to encode an offset of class $k$?
...
$$\log|\mathscr B_{b,k}|=\log\binom{b}{k}\text{ bits}$$
this is because we are counting how many different bit-patterns exist that are $b$ bits long and have $k$ ones.

By summing up the bit-sizes of every offset, we obtain that **the offset $O$ can be packed in**:
$$\sum_{j=1}^{n/b}\log_2\binom{b}{C[j]}\text{ bits}$$
>Where $C[j]$ is the specific value of $k$ (number of $1$s) for the $j$-th block.

**Theorem**:
Our variable -length encoding of $O_0,...,O_{n/b}$ uses $nH_0+o(n)$ bits.
$O_i$ is store using $\lceil\log_2\binom{b}{C_i}\rceil$ bits, and summing over all offsets, we obtain:
$$\begin{align}
\sum_{i=1}^{n/b}\left\lceil\log_2\binom{b}{C_i}\right\rceil&\leq\sum_{i=1}^{n/b}\left(\log_2\binom{b}{C_i}+1\right)\\
...
And recalling that $\log_2\binom{n}{m}\leq nH_0$ we finally obtain that the number of bits is:
...
Array $C$ and table $T$ are stored using a **fixed-length encoding**, for each of their entries, which is easy to access in $O(1)$ time (by the bit-packing theorem).

Let's see how to access any $O[i]$, which is **variable-length** in $O(1)$ time using just $o(n)$ **extra bits**.

The offsets in $O$ are stored as a **flat bitstring with no separators** (e.g. $encoded(O)=01100$), there is not way to know where $O[6]$ starts without knowing where all previous offsets end.
...
but that's too much, since we only want it to occupy only $o(n)$ bits, for this reason we use a **2-level solution**.

**Level 1: macroblocks**
First, we group every $b$ consecutive blocks, together into a **macroblock**, and since $b=\Theta(\log n)$, each macroblock contains $\Theta(\log n)$ blocks.
The total **number of macroblocks** is given by:
$$\frac{n/b}{b}=\frac{n}{b^2}=\frac{n}{\log^2n}$$
...
>[!Tip] Precomputed tables
>The dictionary $T$ is **precomputed**, and the table of encoded offsets and starting positions of macroblocks and offsets inside a macroblock are computed once when the original bitvector $B$ is loaded.

Now let's **build the data structure** recalling our earlier example:
$$B=010\space101\space001\space110\space000\space011\space111$$
...
Now let's **access**, for example $B[18]$:
...
$$\begin{array}{l|ccc|ccc|c}
 & B_0 & B_1 & B_2 & B_3 & B_4 & B_5 & B_6 \\ \hline
...
This means if we concatenate multiple bitvectors and compress the result, the total space is bounded by the sum of their individual entropies, plus small overhead terms.
